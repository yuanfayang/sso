(function (window) {
     ...
     */
    sso.fetchAppList=function fetchAppList(appList) {
        //登录的应用不为空。
        if (appList) {
        ...
        //登出ki4so服务器。
        addScriptTag(ssoServerHost + "/api/logout?callbackName=sso.logoutSsoServer");
    };
     ...
     * @param {Object} data 登录后的数据
     */
    sso.logoutSsoServer=function logoutSsoServer(data) {
        //登出ki4so服务器后的跳转地址。
    ...
    window.logout = function logout() {
        //获取登录的应用列表。
        addScriptTag(ssoServerHost + "/api/getAppList?callbackName=sso.fetchAppList");
